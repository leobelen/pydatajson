...
class PushDatasetTestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.minimum_catalog_id = cls.minimum_catalog.get(
            'identifier',
        ...
        cls.minimum_dataset['distribution'][0]['identifier'] = cls.dataset[
            ...
            if action == 'package_update':
                try:
                    self.assertItemsEqual(
                        keywords, [tag['name'] for tag in data_dict['tags']])
    ...
    def test_dataset_level_wrappers(self, mock_portal):
        ...
        mock_portal.return_value.call_action = mock_call_action
        restored_id = restore_dataset_to_ckan(self.catalog, 'owner',
                                              self.dataset_id, 'portal', 'key')
        harvested_id = harvest_dataset_to_ckan(self.catalog, 'owner',
                                               self.dataset_id, 'portal',
                                               'key', self.catalog_id)
    ...
    def test_harvest_catalog_with_no_optional_parametres(self, mock_portal):
        def mock_call_action(action, data_dict=None):
            if action == 'package_update':
                self.assertTrue(
                    data_dict['id'].startswith(self.catalog_id + '_'))
                self.assertTrue(
                    data_dict['name'].startswith(self.catalog_id + '-'))
        ...
        mock_portal.return_value.call_action = mock_call_action
        harvested_ids = harvest_catalog_to_ckan(self.catalog, 'portal', 'key',
                                                self.catalog_id)
        try:
            self.assertItemsEqual([
                self.catalog_id + '_' + ds['identifier']
    ...
    def test_harvest_catalog_with_dataset_list(self, mock_portal):
        ...
        harvested_ids = harvest_catalog_to_ckan(
            self.catalog,
            'portal',
            'key',
            self.catalog_id,
...
class RemoveDatasetTestCase(unittest.TestCase):
    ...
    @patch('pydatajson.federation.get_datasets')
    @patch('pydatajson.federation.RemoteCKAN', autospec=True)
    def test_remove_through_filters_and_organization(self, mock_portal,
                                                     mock_search):
...
class PushThemeTestCase(unittest.TestCase):
    ...
    def test_function_pushes_theme_by_identifier(self, mock_portal):
        ...
        result = push_theme_to_ckan(
            self.catalog, 'portal_url', 'apikey', identifier='compras')
    ...
    def test_ckan_portal_is_called_with_correct_parametres(self, mock_portal):
        ...
        push_theme_to_ckan(
            self.catalog, 'portal_url', 'apikey', identifier='contrataciones')

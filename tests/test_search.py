...
def pprint(result):
    print(
        text_type(
            json.dumps(
                result, indent=4, separators=(",", ": "), ensure_ascii=False)))
...
class SearchTestCase(unittest.TestCase):
    ...
    def test_fields_meta_field(self, expected_result):
        fields = pydatajson.search.get_fields(self.catalog, meta_field="type")
        pprint(fields)
        self.assertEqual(expected_result, fields)
    ...
    def test_get_distribution(self, expected_result):
        distribution = pydatajson.search.get_distribution(
            self.catalog, title="Convocatorias abiertas durante el año 2015")
        pprint(distribution)
    ...
    def test_get_field(self, expected_result):
        field = pydatajson.search.get_field(
            self.catalog, title="procedimiento_id")
        pprint(field)
    ...
    def test_get_theme(self, expected_result):
        theme = pydatajson.search.get_theme(
            self.catalog, identifier="adjudicaciones")
        pprint(theme)
